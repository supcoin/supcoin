...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars SUPCOIND and SUPCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

SUPCOIND=${SUPCOIND:-${1}/supcoind}
CLI=${SUPCOINCLI:-${1}/supcoin-cli}
...
$SUPCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$SUPCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
