...
export SUPCOINCLI=${BUILDDIR}/qa/pull-tester/run-supcoin-cli
export SUPCOIND=${REAL_SUPCOIND}
...
if [ "x${ENABLE_SUPCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and supcoind must all be enabled"
fi
